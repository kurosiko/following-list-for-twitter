...
const act_tab = browser.tabs.query({active:true},(tabs)=>{
    console.log(tabs)
    browser.tabs.sendRequest(tabs[0].id,{message:"test"},(res)=>{
        console.log(res)
    })
})
/*
...
//フィルタリングじゃなくて検知かな、そこからHTMLの編集(front)をする
browser.webRequest.onCompleted.addListener(d => {
    if (/https:\/\/twitter\.com\/i\/api\/graphql\/.*\/Following.*$/.test(d.url)) {
            ...
            //ここでfrontにMessagingすれば良きかと!!
            //まだてすとしてないけｄ
            //
            //これLiveshareだからデバッグできない//コミットすればいい？
            //おねがい
            //Messageはtab取得までいけたけどそこから送れてるのかが不明→いっさいfrontが反応しない(同時に実行してるのが原因？)
        }
}, { urls: ["<all-urls>"] });